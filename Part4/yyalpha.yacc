...
term: LEFT_PAREN expr RIGHT_PAREN {$$=$expr;}
	| MINUS expr %prec UMINUS {
		...
		emit(uminus, $$, $expr, NULL, 0 , yylineno);
	}
	| NOT expr {
		...
		if($expr->truelist.size()==0 && $expr->falselist.size()==0){
			...
			emit(if_eq, NULL , $2,newexpr_constbool(true) ,0,yylineno);
			emit(jump , NULL , NULL , NULL , 0, yylineno);
}
		...
		if($lvalue != NULL && $lvalue->sym != NULL ){
			if ($lvalue->sym->get_type() == func_USER || $lvalue->sym->get_type() == func_LIB) {
				...
				if ($lvalue->type == tableitem_e) {
					...
					emit(add,$lvalue, $lvalue, newexpr_constnum(1),0,yylineno );
		...
		if($lvalue != NULL && $lvalue->sym != NULL ){
			if ($lvalue->sym->get_type() == func_USER || $lvalue->sym->get_type() == func_LIB) {
				...
				if ($lvalue->type == tableitem_e) {
					...
					emit(assign,$term, val, NULL,0,yylineno );
		...
		if($lvalue != NULL && $lvalue->sym != NULL ){
			if ($lvalue->sym->get_type() == func_USER || $lvalue->sym->get_type() == func_LIB) {
				...
				if ($lvalue->type == tableitem_e) {
					...
					emit(sub,$lvalue, $lvalue, newexpr_constnum(1),0,yylineno );
					$term = newexpr(arithexpr_e);
					$term->sym = new_temp();
					emit(assign,$$, $lvalue, NULL,0,yylineno);
...
assignexpr: lvalue {
		...
		if ($1 != NULL && $expr->inLogical) {
		    ...
		    emit(assign, $assignexpr, newexpr_constbool(true), NULL, 0, yylineno);
		    emit(jump, NULL, NULL, NULL, nextquad() + 2, yylineno);
		    emit(assign, $assignexpr, newexpr_constbool(false), NULL, 0, yylineno);
		    isBackpatch = true;
		}
auto a = $lvalue;
...
objectdef: LEFT_BRACKET indexed RIGHT_BRACKET  {
		...
		for (int i = 0; $indexed; $indexed = $indexed->next) {
		    emit(tablesetelem, temp, $indexed->index, $indexed->element, 0, yylineno);
		}
		$$ = temp;
	 }
	 | LEFT_BRACKET  elist  RIGHT_BRACKET {
        ...
        for (int i = 0; $elist; $elist = $elist->next){
            emit(tablesetelem,  temp, newexpr_constnum(i++), $elist , 0 , yylineno);
        }
        $$ = temp;
	}
	;
...
funcdef: func_prefix func_args func_body{
		...
		$funcdef = $func_prefix;
		emit(funcend, $funcdef, NULL, NULL , 0 ,yylineno);
...
if ($expr!=NULL && $expr->inLogical){
			$expr = newexpr(assignexpr_e);
			$expr->sym = new_temp();
			emit(assign , $expr , newexpr_constbool(true) , NULL ,  0 , yylineno);
			emit(jump   , NULL, NULL , NULL , nextquad() + 2  , yylineno);
			emit(assign , $expr  , newexpr_constbool(false) , NULL , 0 , yylineno);
}
			emit(if_eq, NULL, $expr, newexpr_constbool(true),  nextquad()+2, yylineno);
			$$ = nextquad();
			emit(jump,NULL, NULL, NULL, 0, yylineno);
			/* isBackpatch = true; */
	}
...
forprefix: FOR { loop_enter(); } LEFT_PAREN elist SEMICOLON M expr SEMICOLON	{
		...
		emit(if_eq,NULL , $expr, newexpr_constbool(true),  0 , yylineno);
		isBackpatch = 0;
			}
